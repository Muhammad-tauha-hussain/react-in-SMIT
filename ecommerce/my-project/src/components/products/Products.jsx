            ...
            <div className="flex mt-6">
                ...
                <div className='w-3/4  p-4 flex gap-4 flex-wrap  relative h-[25rem] overflow-y-scroll scroll-smooth ' ref={parent}>
                    {Menuproducts.map((item, index) => (
                        <div key={index} className="flex justify-between w-[15rem] bg-white rounded-lg p-4 shadow-lg relative overflow-hidden">
                            <div className="text w-[60%]">
                                <h1 className='text-lg font-bold'>{item.name}</h1>
                                <p className='text-sm'>{item.detail}</p>
                                <h1 className='font-semibold mt-2'>{item.price}$</h1>
                                <button className='border border-black p-2 rounded-lg mt-2'>Shop now</button>
                            </div>
